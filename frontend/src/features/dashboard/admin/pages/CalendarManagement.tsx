/**
 * Страница управления календарем событий (администратор)
 * 
 * Функциональность:
 * - Создание событий соревнований (расписание, церемонии)
...
import { Calendar, Card, List, Tag, Space, Modal, Form, Select, TimePicker, DatePicker, Button, message, Popconfirm, Input, Tabs } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, TrophyOutlined, CalendarOutlined } from '@ant-design/icons';
...
const getRoundLabel = (round: number, totalRounds: number): string => {
  ...
  // Используем ту же логику, что и в generate-bracket-simple.ts
...
export const CalendarManagement = () => {
    ...
    const handleBracketMatchUpdated = () => {
      if (selectedCompetition) {
        ...
        try {
          ...
          console.warn('Ошибка при обновлении матчей после события (не критично):', error);
        }
      }
    };
  ...
  const cellRender = (value: Dayjs, info: any) => {
    // Для отображения в ячейках дат используем dateCellRender
    if (info.type === 'date') {
  ...
  const handleEditMatch = (match: Match) => {
    ...
    const matchDate = match.scheduledTime ? dayjs(match.scheduledTime) : selectedDate;
  ...
  const handleCreateEventFromCalendar = (date: Dayjs) => {
    if (!selectedCompetition) {
      message.warning('Выберите соревнование');
      return;
    }
    
    message.info('Выберите матч из календаря для установки расписания');
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      // Если редактируем существующий матч
      if (editingMatch) {
        // Используем дату из формы, если указана, иначе используем selectedDate
        const dateToUse = values.scheduledDate || selectedDate;
        const dateOnly = dateToUse.format('YYYY-MM-DD');
        const timeOnly = values.scheduledTime.format('HH:mm');
        const scheduledDateTime = dayjs(`${dateOnly} ${timeOnly}`).toISOString();
        
        await apiClient.put(`/brackets/match/${editingMatch.id}`, {
          scheduledTime: scheduledDateTime,
        });
        message.success('Расписание схватки обновлено');
      } else {
        message.warning('Для создания события выберите матч из календаря');
      }
  ...
  const handleSubmitEvent = async (values: any) => {
    ...
    try {
      ...
      const dateOnly = dateToUse.format('YYYY-MM-DD');
      const timeOnly = values.scheduledTime.format('HH:mm');
      const startDateTime = dayjs(`${dateOnly} ${timeOnly}`).toISOString();
      
      const endDateTime = values.endTime 
        ? dayjs(`${dateOnly} ${values.endTime.format('HH:mm')}`).toISOString()
        : null;

      if (editingEvent) {
        await apiClient.put(`/competitions/events/${editingEvent.id}`, {
          title: values.title,
        ...
        await apiClient.post(`/competitions/${selectedCompetition}/events`, {
          title: values.title,
      ...
      message.error(error.response?.data?.message || 'Ошибка при сохранении мероприятия');
    }
  };

  const handleDeleteEvent = async (eventId: string) => {
    try {
      await apiClient.delete(`/competitions/events/${eventId}`);
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: '28px', fontWeight: 600, margin: 0, color: '#262626' }}>Управление календарем соревнований</h1>
        <Select
      ...
      {selectedCompetition ? (
        <Space direction="vertical" style={{ width: '100%' }} size="large">
          ...
          >
            <Tabs
              ...
              items={[
                {
                  key: 'matches',
                  label: `Схватки (${selectedDateMatches.length})`,
                  children: selectedDateMatches.length > 0 ? (
                    <List
                      loading={loading}
                  ...
                  <p style={{ textAlign: 'center', color: '#8c8c8c', padding: '20px' }}>
                    На эту дату нет запланированных схваток
                  </p>
                ),
                },
                {
                  key: 'events',
                  label: `Мероприятия (${selectedDateEvents.length})`,
                  children: selectedDateEvents.length > 0 ? (
                    <List
                      loading={loading}
          ...
          {getMatchesWithoutSchedule().length > 0 && (
            <Card 
              title="Схватки без расписания"
              extra={
                <Button
                  type="primary"
                  icon={<PlusOutlined />}
                  onClick={() => {
                    if (getMatchesWithoutSchedule().length > 0) {
                      handleEditMatch(getMatchesWithoutSchedule()[0]);
                    }
                  }}
                >
                  Добавить расписание
        ...
        <Card>
          <p>Выберите соревнование для управления календарем</p>
        </Card>
      )}

      <Modal
        title={editingMatch ? 'Редактировать расписание схватки' : 'Создать событие'}
        open={modalVisible}
        onCancel={() => {
          setModalVisible(false);
          setEditingMatch(null);
          ...
          {editingMatch && (
            <>
              ...
              <Form.Item label="Весовая категория">
                <Tag>{editingMatch.bracket.weightCategory.name}</Tag>
              </Form.Item>
            </>
          )}
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="Например: Торжественное открытие" />
          </Form.Item>
          ...
          <Form.Item
            ...
            label="Описание"
          >
            <Input.TextArea rows={3} placeholder="Описание мероприятия (необязательно)" />
          </Form.Item>
          ...
          >
            <TimePicker
              ...
              style={{ width: '100%' }}
              showNow
              placeholder="Выберите время окончания (необязательно)"
            />
          </Form.Item>
          ...
          >
            <Input placeholder="Место проведения мероприятия (необязательно)" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
